...
class Php5 {
	...
	public function __construct($meta, $targetPath) {
		...
		$this->_tplPath = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'Template' . DIRECTORY_SEPARATOR . 'Php5' . DIRECTORY_SEPARATOR;
		$this->_targetPath = $targetPath;
		
		if (!is_dir($this->_targetPath . 'baseModel')) {
			mkdir($this->_targetPath . 'baseModel', 0777);
		}
	}
	...
	private function _generateBaseFiles(\Cwa\Library\Orm\Generator\Object\Table $table) {
		...
		return file_put_contents($this->_targetPath . 'baseModel/' . ucfirst($table->getName()) . 'Base.php', $string);
	}
	...
	private function _generateQueryFiles(\Cwa\Library\Orm\Generator\Object\Table $table) {
		...
		return file_put_contents($this->_targetPath . 'baseModel/' . ucfirst($table->getName()) . 'Query.php', $string);
	}
	...
	public function setRuntimeNamespace() {}
