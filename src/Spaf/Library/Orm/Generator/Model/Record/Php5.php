 ...
 */ 
class Php5 extends \Spaf\Library\Orm\Generator\Model\Php5 {
			...
			if ($t->isRelation()) {
				...
				if ($cols !== null) {
					...
					if ($self !== null && $foreign !== null && $isFrom === true) {
						...
						$methods .= "\t\t" . 'if (get_class($c) !== \'DbCriteria\') {' . "\n";
						...
						if ($self === $foreign) {
							...
							$methods .= "" . '' . "\n";
							$methods .= "\t\t" . 'return ' . ucfirst($foreign) . 'Query::doSelect($c);' . "\n";
						...
						$methods .= "\t\t" . '$_obj = array();' . "\n";
						$methods .= "\t\t" . 'if (!is_array($obj) && get_class($obj) === \'' . ucfirst($foreign) . '\') {' . "\n";
						$methods .= "\t\t\t" . '$_obj[0] = $obj;' . "\n";
						$methods .= "\t\t" . '} else if (!is_array($obj)) {' . "\n";
						$methods .= "\t\t\t" . 'throw new DbException(\'' . ucfirst($self) . '::set' . ucfirst($inf->pluralize($foreign)) . '() -- Just a ' . ucfirst($foreign) . ' object or a array of ' . ucfirst($foreign) . ' objects accepted as param. \' . gettype($obj) . \' given!\');' . "\n";
						$methods .= "\t\t" . '} else {' . "\n";
						$methods .= "\t\t\t" . '$_obj = $obj;' . "\n";
						$methods .= "\t\t" . '}' . "\n";
						$methods .= "\t\t" . '$obj = $_obj;' . "\n";
	...
	private function _getOneToManyMethods(\Spaf\Library\Orm\Generator\Object\Table $table) {
		...
		if ($table->getFkColumns() !== null) {
			...
			foreach  ($cols as $key => $value) {
				...
				$methods .= "\t\t" . 'return ' . ucfirst($key) . 'Query::retrieveByPk(array(';
				// Zähler für Verknüpfung
				$i = 0;
		...
		foreach ($this->_meta as $t) {
			...
			if ($cols !== null) {
				...
				foreach  ($_cols as $key => $value) {
					...
					$methods .= "\t\t" . '// @todo			check existing DbCriteria object for specific queries' . "\n";
					$methods .= "\t\t" . 'if (!$c instanceof DbCriteria) {' . "\n";
					$methods .= "\t\t\t" . '$c = new DbCriteria();' . "\n";
					...
					$methods .= "\t\t" . 'return ' . ucfirst($t->getName()) . 'Query::doSelect($c);' . "\n";
	...
	private function _getOneToOneMethods(\Spaf\Library\Orm\Generator\Object\Table $table) {
		...
		if ($table->getFkColumns() !== null) {
			...
			foreach  ($cols as $key => $value) {
				$methods .= "\n\t" . 'public function set' . ucfirst($key) . '(' . ucfirst($key) . ' $obj) {' . "\n";
				/*
				if ($obj->getUserDetails() !== null) {
					throw new DbException('This user has already a relation. Since this is a 1:1 relation, this is not allowed!');
				}
				*/
				$methods .= "\t\t" . 'if ($obj->get' . ucfirst($table->getName()) . '() !== null && $obj->get' . ucfirst($table->getName()) . '() !== $this) {' . "\n";
				$methods .= "\t\t\t" . 'throw new DbException(\'' . ucfirst($table->getName()) . ':: Param ' . $key . ' has already a relation. Since it\\\'s a 1:1 relation, this operation is not allowed!\');' . "\n";
				$methods .= "\t\t" . '}' . "\n";
		...
		foreach ($this->_meta as $key => $t) {
			...
			if ($cols !== null) {
				...
				foreach  ($_cols as $key => $value) {
					...
					$methods .= "\t\t" . '$objs = ' . ucfirst($t->getName()) . 'Query::doSelect($c);' . "\n";
			...
			if ($t->isRelation()) {
				...
				if ($cols !== null) {
					...
					if ($self !== null && $foreign !== null && $isFrom === true) {
						...
						$delStr .= "\t\t" . '} catch (DbDriverException $e) {' . "\n";
						$delStr .= "\t\t\t" . '// rollback transaction and throw DbDriverException' . "\n";
						$delStr .= "\t\t\t" . 'self::$_dbh->rollback();' . "\n";
						$delStr .= "\t\t\t" . 'throw new DbDriverException($e->getMessage());' . "\n";
						$delStr .= "\t\t" . '}' . "\n";
