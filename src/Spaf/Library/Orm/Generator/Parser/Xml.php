 ...
 * @reviewer 	TODO
 * @package		\Spaf\Library\Database\Generator
 * @namespace	\Spaf\Library\Database\Generator\Parser
 */
namespace Spaf\Library\Orm\Generator\Parser;

/**
 * Spaf\Library\Orm\Generator\Parser\Xml
 *
...
class Xml extends AbstractParser {
	 ...
	 *
	 * @param		\Spaf\Library\Directory\File			The source XML as file object
	 * @return		boolean
	 */
	protected function _loadFile(\Spaf\Library\Directory\File $xmlFile) {
	...
	private function _readConfigObject() {
		$this->_config = new \Spaf\Library\Orm\Generator\Object\Config();
	...
	private function _readLazyObjects() {
		foreach ($this->_xml->table as $t) {
			$attr = $t->attributes();
			$tablename = $attr->name;
			$table = new \Spaf\Library\Orm\Generator\Object\Table($attr->name, $attr->description);

			foreach ($t->column as $c) {
				$attr = $c->attributes();
				$column = new \Spaf\Library\Orm\Generator\Object\Column($attr->name);
	 ...
	 *
	 * @param		\Spaf\Library\Orm\Generator\Object\Table			A Table object
	 * @return		array											Array of the foreignColumns from this table
	 */
	private function _getForeignColumns(\Spaf\Library\Orm\Generator\Object\Table $table) {
		$cols = array();
		foreach ($this->_xml->table as $t) {
			foreach ($t->has as $_dep) {
		...
		foreach ($this->_xml->table as $t) {
			foreach ($t->has as $_dep) {
				...
				if ((string) $attr->relation === 'n:m') {
					$table = new \Spaf\Library\Orm\Generator\Object\Table($t->attributes()->name . 'To' . ucfirst($attr->table), 'autogenerated dependency table');
